...
void	drawing_wall_right_fov(t_fenetre *s, int x, int y, double wall_height)
{
	int		y_index;
	...
	k = 0;
	y_index = 0;
	angle = s->pa - ((x - s->longeur / 2) * s->fov_angle);
...
void	draw_right_fov(t_fenetre *s)
{
	...
	angle = s->pa;
	x = s->longeur / 2;
	mlx_clear_window(s->id, s->win);
	while (x < s->longeur)
	{
		r = ray_draw(s, angle) * cos(angle - s->pa);
		u = ((64 / r) * s->projection);
		ywalls = (s->largeur / 2) - (u / 2);
		drawing_wall_right_fov(s, x, ywalls, u);
		drawing_ceil_right_fov(s, x, ywalls);
		drawing_floor_right_fov(s, x, (ywalls + u));
		angle -= s->fov_angle;
		x++;
	}
}
